...
- **Rider safe delete remains intentionally excluded, and Rider F# is still beta/unstable.** The docs now state that `ide_refactor_safe_delete` is not exposed for Rider by design, and that F# is not production-ready until Rider language support matures further.
...
### Changed
- **Rider C# mutation paths now report bounded, backend-verified outcomes.** Symbol rename, file rename, and move for Rider-backed C# use the backend routing/verification path with exact-target safety and explicit `success` / `no_op` / `needs_active_editor` / `conflict` / `unsupported_context` / `failed` external statuses, while any legacy verification details remain confined to metadata/trace rather than terminal outcomes. Safe delete remains intentionally excluded from Rider.
...
- **C# / F# rename and move use ReSharper's refactoring engine.** `ide_refactor_rename` for C# and F# acquires a backend write lock and runs ReSharper's rename workflow headlessly — no lexical search/replace fallback. `ide_move_file` adjusts namespaces and imports through ReSharper when moving `.cs` files between project folders. Safe delete remains Java/Kotlin-only and is intentionally not exposed in Rider.
...
- **Rider C# / F# rename resolves class declarations and updates references through the real ReSharper workflow.** Type declarations now participate in local declaration resolution, path-to-PSI lookup prefers project-backed PSI files over miscellaneous/plain representations, and the backend initializes ReSharper rename with an explicit compilation context so real C# types such as `DstsAuthenticationMetadata` can be renamed with references.
...
- **Rider backend NuGet restore conflict in CI.** Added a direct `System.Diagnostics.TraceSource` reference so `dotnet build` resolves the Rider SDK's transitive `TraceSource` version conflict consistently.
